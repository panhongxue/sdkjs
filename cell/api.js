		...
		let doInsertXml = function (_dataRef, _ws) {
			...
			if (wb.aWorksheets) {
				...
				if (pastedSheet) {
					...
					t.wb.getWorksheet().setSelectionInfo('paste', {
						data: pastedSheet,
						fromBinary: true,
						fontsNew: [],
						pasteAllSheet: true,
   ...
   */
	...
	spreadsheet_api.prototype._onNeedParams = function (data, opt_isPassword) {
		...
		if (this.documentOpenOptions && !opt_isPassword) {
			var codePageCsv = AscCommon.c_oAscEncodingsMap[this.documentOpenOptions["codePage"]] || AscCommon.c_oAscCodePageUtf8,
				...
				wbXml.sheets.forEach(function (wbSheetXml) {
					if (null !== wbSheetXml.id && wbSheetXml.name) {
						...
						wsParts.push({
							wsPart: wsPart,
							id: wbSheetXml.id,
							name: wbSheetXml.name,
							bHidden: wbSheetXml.bHidden,
							sheetId: wbSheetXml.sheetId
						});
					}
				});
...
//      if( lockElem.Element["subType"] == AscCommonExcel.c_oAscLockTypeElemSubType.DefinedNames ){
	...
	spreadsheet_api.prototype.asc_setListType = function (type, subtype, custom) {
		...
		if (typeof sNeedFont === "string" && sNeedFont.length > 0) {
			...
			fonts[sNeedFont] = 1;
			t._loadFonts(fonts, function () {
				t.asc_setListType2(type, subtype, custom);
			});
		} else {
	...
	spreadsheet_api.prototype.asc_nativeGetFile3 = function () {
		...
		return {
			...
			header: oBinaryFileWriter.WriteFileHeader(oBinaryFileWriter.Memory.GetCurPosition(), Asc.c_nVersionNoBase64)
		};
	};
	...
	spreadsheet_api.prototype.asc_setProtectedSheet = function (props) {
		...
		if (props && props.temporaryPassword != null) {
			if (props.temporaryPassword === "") {
				...
				var checkHash = {
					password: props.temporaryPassword, salt: props.saltValue, spinCount: props.spinCount,
	...
	spreadsheet_api.prototype.asc_setProtectedWorkbook = function (props) {
		...
		if (props && props.temporaryPassword != null) {
			if (props.temporaryPassword === "") {
				...
				var checkHash = {
					password: props.temporaryPassword,
					salt: props.workbookSaltValue,
					spinCount: props.workbookSpinCount,
					alg: AscCommon.fromModelAlgorithmName(props.algorithmName)
				};
				AscCommon.calculateProtectHash([checkHash], checkPassword);
			}
	...
	spreadsheet_api.prototype.asc_openExternalReference = function (externalReference) {
		...
		if (isLocalDesktop) {
			window["AscDesktopEditor"]["openExternalReference"](externalReference.externalReference.Id, function (error) {
				...
				switch (error) {
					case 0:
						internalError = Asc.c_oAscError.ID.ConvertationOpenError;
						break;
					default:
						break;
				}
