...
(
	...
	function (window, undefined) {
		...
		CellTextRender.prototype.calcLineOffset = function (index) {
			if (index < 0) {index = 0;}
			if (index > this.getLinesCount()) {index = this.getLinesCount();}
			
			let dZoom = this.drawingCtx.getZoom();
			let nOffset = 0;
			let aLines = this.getLines();
			for(let nLine = 0; nLine < aLines.length; ++nLine) {
				let oLine = aLines[nLine];
				let oMetrics = oLine.Metrics;
				let dHeight = oMetrics.TextAscent + oMetrics.TextDescent;
				nOffset += this.mmToPixels(dHeight);
			}
			return nOffset;
		};
		...
		CellTextRender.prototype.getBeginOfLine = function (pos) {
			...
			// return this.getCharWidth(lastChar) !== 0 ? l[lastLine].beg : pos;
		};
		...
		CellTextRender.prototype.getEndOfLine = function (pos) {
			...
			var lastChar = this.chars.length - 1;
			return pos > lastChar ? pos : lastChar + (this.getCharWidth(lastChar) !== 0 ? 1 : 0);
		};
		...
		CellTextRender.prototype.getPrevLine = function (pos) {
			...
			// var lastLine = l.length - 1;
			// var lastChar = this.chars.length - 1;
			// return this.getCharWidth(lastChar) === 0 || l.length < 2 ?
		...
		CellTextRender.prototype.charOffset = function (pos, lineIndex, h) {
			return new CharOffset(0, 0, 0, lineIndex);
		...
		CellTextRender.prototype.calcCharOffset = function (pos) {
			...
			let co = this.charOffset(pos, 0, 0);
			return co;
		};
